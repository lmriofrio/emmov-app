    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.success) {
                ...
                response.tramitesUsuario.forEach(tramite => {
                    ...
                    const newRow = `
                        <tr style="">
                            ...
                            <td class="text-center d-none">${tramite.id_tramite}</td>
                    ...
                    $('#tbody-tramites').off('click', '.editarTramite').on('click', '.editarTramite', function () {
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                if (response.success) {
                                    window.location.href = `/matriculacion/informacion/editar-turno?id_tramite=${id_tramite}`;
                                } else {
                                    alert('Error: no se pudo obtener la información del trámite.');
                                }
                            },
                ...
                let numeroFilaG = 1;
                response.tramitesGeneral.forEach(tramite => {
                ...
                response.tramitesPendientesEmpresa.forEach(tramite => {
                    ...
                    `;
                    tbodyEmpresa.append(newRow2);
                    numeroFilaE++;

                    $('#tbody-tramitesEmpresa').off('click', '.editarTramite').on('click', '.editarTramite', function () {
                        const idTramite = $(this).data('id-tramite');
                        const id_tramite = idTramite;
                ...
                $('#TraGeneralEmpresa').text(`${response.tramitesPendientesEmpresa.length}`);
